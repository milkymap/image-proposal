...
class ZMQRunner:
    ...
    def launch_server(self, host:str, port:int, mounting_path:str, index_name:str, reset_index:bool, path2base_dir:str, es_host:str, es_port:int, es_scheme:str, es_basic_auth:str, path2index_schema:str):
        async def start_server(host:str, port:int, index_name:str, reset_index:bool, path2base_dir:str, mounting_path:str):
            async with APIServer(host=host, port=port, path2base_dir=path2base_dir, index_name=index_name, reset_index=reset_index, number_opened_file_limit=8192, mounting_path=mounting_path) as server:
                await server.loop(es_host, es_port, es_scheme, es_basic_auth, path2index_schema)
        
        asyncio.run(start_server(host, port, index_name, reset_index, path2base_dir, mounting_path))
