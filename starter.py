...
def launch_server(host:str, port:int, path2base_dir:str, es_host:str, es_port:int, es_scheme:str, es_basic_auth:str, path2index_schema:str):
    async def start_server(host:str, port:int, path2base_dir:str):
        async with APIServer(host, port, path2base_dir, number_opened_file_limit=8192) as server:
            await server.loop(es_host, es_port, es_scheme, es_basic_auth, path2index_schema)
    
    asyncio.run(start_server(host, port, path2base_dir))
